...
class ExportFBX(bpy.types.Operator, ExportHelper, IOFBXOrientationHelper):
    ...
    ui_tab = EnumProperty(
            items=(('MAIN', "Main", "Main basic settings"),
				   ...
				   ('MATERIALS', "Materials", "Material-related settings"), #added this shit
                   ),
            name="ui_tab",
    ...
    # End anim
    #7.4 materials //add here
    del_materials = BoolProperty(
            name="Delete Materials",
            description="Delete materials to export with just one textureset (mainly for substance painter)",
            default=False,
            )
	# End materials
    ...
    def draw(self, context):
        ...
        if self.version == 'BIN7400':
            ...
            if self.ui_tab == 'MAIN':
                ...
                layout.prop(self, "del_materials")
        else:
            layout.prop(self, "use_selection")
            layout.prop(self, "global_scale")
            layout.prop(self, "axis_forward")
