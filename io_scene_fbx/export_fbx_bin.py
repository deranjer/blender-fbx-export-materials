...
def fbx_data_material_elements(root, mat, scene_data):
    ...
    if not skip_mat and not scene_data.settings.del_materials:
...
def fbx_data_from_scene(scene, settings): #modified
    """
    ...
    for ob_obj in objects:
        ...
        for mat_s in ob_obj.material_slots:
            mat = mat_s.material
            if mat is None or settings.del_materials: #added to delete materials from mesh before export
...
def save_single(operator, scene, filepath="",
    ...
    settings = FBXExportSettings(
        ...
        False, media_settings, use_custom_props, del_materials
    )
